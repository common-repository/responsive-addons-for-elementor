...
class Module extends Module_Base {
	...
	public function ajax_posts_filter_autocomplete( array $data ) {
		...
		switch ( $data['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'rael/core_elements/query_control/get_autocomplete/' . $data['filter_type'], array(), $data );
		}
	...
	public function ajax_posts_control_value_titles( $request ) {
		...
		switch ( $request['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'rael/core_elements/query_control/get_value_titles/' . $request['filter_type'], array(), $request );
		}
	...
	public function rael_get_posts_by_terms() {
		if ( isset( $_POST['data']['term'] ) && '' !== $_POST['data']['term'] ) {
			$term      = isset( $_POST['data']['term'] ) ? $_POST['data']['term'] : '';
			$post_id   = isset( $_POST['data']['pid'] ) ? $_POST['data']['pid'] : '';
			$widget_id = isset( $_POST['data']['widget_id'] ) ? $_POST['data']['widget_id'] : '';
			$style_id  = isset( $_POST['data']['skin'] ) ? $_POST['data']['skin'] : '';
